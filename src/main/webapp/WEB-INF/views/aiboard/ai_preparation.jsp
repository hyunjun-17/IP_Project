...
<script>
        ...
        try {
            ...
            const selectedQuestions = Array.from(document.querySelectorAll('input[name="selectedQuestions"]:checked'))
                .map((checkbox, index) => {
                    ...
                    const iproIdx = checkbox.getAttribute('data-ipro-idx');
            ...
            currentInterviewId = data.id; // 여기서 ID 설정

            // UI 업데이트 전에 트랜스크립트 리스트 업데이트
    ...
    function updateCurrentQuestion(questionNumber) {
        console.log('Updating to question number:', questionNumber); // 디버깅 로그 추가

        const transcriptItems = document.querySelectorAll('.transcript-item');
        console.log('Available transcript items:', transcriptItems.length); // 디버깅 로그 추가

        const targetItem = transcriptItems[questionNumber - 1];
        console.log('Target item:', targetItem); // 디버깅 로그 추가
